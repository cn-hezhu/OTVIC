...
from mmdet3d.core.bbox import Box3DMode, DepthInstance3DBoxes, LiDARInstance3DBoxes
...
def test_part_aggregation_ROI_head():
    ...
    proposal = dict(
        boxes_3d=pred_bboxes, scores_3d=pred_scores, labels_3d=pred_labels, cls_preds=pred_clses
    )
...
def test_free_anchor_3D_head():
    ...
    gt_bboxes = [
        LiDARInstance3DBoxes(torch.rand([8, 9], device="cuda"), box_dim=9) for i in range(4)
    ]
...
def test_primitive_head():
    ...
    primitive_head_cfg = dict(
        ...
        objectness_loss=dict(
            type="CrossEntropyLoss", class_weight=[0.4, 0.6], reduction="mean", loss_weight=1.0
        ),
        center_loss=dict(
