...
class SSTInputLayer(nn.Module):
    ...
    def forward(self, voxel_feat, coors):
        ...
        voxel_info = self.get_voxel_keep_inds(
            voxel_info, len(self.shifts_list)
        ...
        flat2win_inds_list = [
            self.get_flat2win_inds(
                voxel_info[f"batch_win_inds_shift{i}"], voxel_info[f"voxel_drop_level_shift{i}"]
            )
        ...
        if self.debug:
            coors_3d_dict_shift0 = flat2window(
                coors, voxel_info["voxel_drop_level_shift0"], flat2win_inds_list[0], self.drop_info
            )
    ...
    def window_partition(self, coors, voxel_info):
        ...
        max_num_win_x = int(
            np.ceil((bev_shape_x / win_shape_x)) + 1
        )  # plus one here to meet the needs of shift.
        max_num_win_y = int(
            np.ceil((bev_shape_y / win_shape_y)) + 1
        )  # plus one here to meet the needs of shift.
        ...
        for i in range(len(shifts_list)):
            ...
            batch_win_inds = (
                coors[:, 0] * max_num_win_per_sample + win_coors_x * max_num_win_y + win_coors_y
            )
