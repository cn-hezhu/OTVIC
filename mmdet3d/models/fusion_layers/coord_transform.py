...
def apply_3d_transformation(pcd, coords_type, img_meta, reverse=False):
    ...
    horizontal_flip_func = (
        partial(pcd.flip, bev_direction="horizontal") if pcd_horizontal_flip else lambda: None
    )
    vertical_flip_func = (
        partial(pcd.flip, bev_direction="vertical") if pcd_vertical_flip else lambda: None
    )
    if reverse:
...
def bbox_2d_transform(img_meta, bbox_2d, ori2new):
    ...
    img_h, img_w, ori_h, ori_w, img_scale_factor, img_flip, img_crop_offset = extract_2d_info(
        img_meta, bbox_2d
    )
