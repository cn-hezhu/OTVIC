...
class VoteFusion(nn.Module):
    ...
    def forward(self, imgs, bboxes_2d_rescaled, seeds_3d_depth, img_metas, calibs):
        """Forward function.
        ...
        for i, data in enumerate(zip(imgs, bboxes_2d_rescaled, seeds_3d_depth, img_metas)):
            ...
            xyz_cam = Coord3DMode.convert_point(
                xyz_depth, Coord3DMode.DEPTH, Coord3DMode.CAM, rt_mat=calibs["Rt"][i]
            )
            ...
            if bbox_num == 0:
                ...
                z_cam = xyz_cam[..., 2:3].view(seed_num, 1, 1).expand(-1, bbox_num, -1)

                delta_u = delta_u * z_cam / calibs["K"][i, 0, 0]
                delta_v = delta_v * z_cam / calibs["K"][i, 0, 0]

                imvote = torch.cat([delta_u, delta_v, torch.zeros_like(delta_v)], dim=-1).view(
                    -1, 3
                )

                # convert from camera coords to depth coords
                imvote = Coord3DMode.convert_point(
                    imvote.view((-1, 3)), Coord3DMode.CAM, Coord3DMode.DEPTH, rt_mat=calibs["Rt"][i]
                )
