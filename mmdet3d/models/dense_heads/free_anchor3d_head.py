...
class FreeAnchor3DHead(Anchor3DHead):
    ...
    def __init__(self, pre_anchor_topk=50, bbox_thr=0.6, gamma=2.0, alpha=0.5, **kwargs):
    ...
    def loss(
        ...
        cls_scores = [
            cls_score.permute(0, 2, 3, 1).reshape(cls_score.size(0), -1, self.num_classes)
            for cls_score in cls_scores
        ]
        bbox_preds = [
            bbox_pred.permute(0, 2, 3, 1).reshape(bbox_pred.size(0), -1, self.box_code_size)
        ...
        for _, (
            ...
            bbox_preds_,
            dir_cls_preds_,
        ) in enumerate(zip(anchors, gt_labels, gt_bboxes, cls_prob, bbox_preds, dir_cls_preds)):
            ...
            with torch.no_grad():
                ...
                if indices.numel() == 0:
                    image_box_prob = torch.zeros(anchors_.size(0), self.num_classes).type_as(
                        object_box_prob
                    )
                else:
                    nonzero_box_prob = (
