...
class BEVF_V2IFusion(MVXFasterRCNN):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        super(BEVF_V2IFusion, self).__init__(**kwargs)
        ...
        self.lidar_stream = lidar_stream
        self.lss = lss
        self.se = se
    ...
    def extract_img_bev_feat_by_lss(self, img, img_metas):
        ...
        for i, img_i in enumerate(img):
            num_views_per_sample.append(img_i.shape[0])
        ...
        for sample_idx, img_feats in enumerate(img_feats_list):
            ...
            img_bev_feat, depth_dist = self.lift_splat_shot_vis(
                img_feats_view, rots, trans, img_metas=img_metas
            )
    ...
    def obtain_history_bev(self, imgs_queue, points_queue, img_metas_list):
        ...
        with torch.no_grad():
            ...
            bs, len_queue, N, D = points_queue.shape
            assert len_queue == 1
            points_queue = points_queue.reshape(bs*len_queue, N, D)
            img_metas = [each[len_queue] for each in img_metas_list]
            prev_bev = self.extract_feat(points=points_queue, img=imgs_queue, img_metas=img_metas)
            # for i in range(len_queue):
            #     img_metas = [each[i] for each in img_metas_list]
            #     prev_bev = self.extract_feat(points=points_queue, img=imgs_queue, img_metas=img_metas)
            self.train()
            return prev_bev["pts_feats"]
    ...
    def _forward_train(
        ...
        all_img_metas = copy.deepcopy(img_metas)
        prev_bev = self.obtain_history_bev(prev_img, prev_points, all_img_metas)
        ...
        if pts_feats:
            ...
            losses_pts = self.forward_pts_train(
                pts_feats, gt_bboxes_3d, gt_labels_3d, img_metas, gt_bboxes_ignore, prev_bev, prev_img_metas
            )
    ...
    def forward_pts_train(
        self, pts_feats, gt_bboxes_3d, gt_labels_3d, img_metas, gt_bboxes_ignore=None, prev_bev=None, prev_img_metas = None
    ):
        ...
        """
        outs = self.pts_bbox_head(pts_feats, img_metas, prev_bev, prev_img_metas)
    ...
    def simple_test_pts(self, x, img_metas, prev_bev=None, prev_img_metas=None, rescale=False):
        """Test function."""
        outs = self.pts_bbox_head(x, img_metas, prev_bev, prev_img_metas)

        bbox_list = self.pts_bbox_head.get_bboxes(outs, img_metas, rescale=rescale)
        bbox_results = [
            bbox3d2result(bboxes, scores, labels) for bboxes, scores, labels in bbox_list
        ]
        return outs[0][0]['bev_feat_flatten'], bbox_results
    
    def simple_test(self, points, img_metas, img=None, prev_bev=None, prev_img_metas=None, rescale=False):
        ...
        if pts_feats is not None and self.with_pts_bbox:
            new_prev_bev, bbox_pts = self.simple_test_pts(pts_feats, img_metas, prev_bev, prev_img_metas, rescale=rescale)
    ...
    def _forward_test(self, points, img_metas, img=None, **kwargs):
        ...
        prev_img_metas = [[{
            'road_info':self.prev_frame_info['road_info'],
            'timestamp':self.prev_frame_info['timestamp']
            }]]
        
        # update idx
        self.prev_frame_info['scene_token'] = img_metas[0][0]['scene_token']
        self.prev_frame_info['road_info'] = img_metas[0][0]['road_info']
        self.prev_frame_info['timestamp'] = img_metas[0][0]['timestamp']
        ...
        if num_augs == 1:
            ...
            new_prev_bev, bbox_results = self.simple_test(points[0], img_metas[0], img[0], prev_bev=self.prev_frame_info['prev_bev'], prev_img_metas=prev_img_metas[0],**kwargs)
