...
class OTVICDetectionEval(DetectionEval):
    def __init__(
        self,
        data_infos,
        config,
...
class OTVICDataset(Custom3DDataset):
    CLASSES = (
        "car",
        "truck",
    )
    ...
    def union2one(self, queue):
        ...
        if self.modality["use_lidar"]:
            points_data_list = [each["points"].data for each in queue]
            points_data_list = torch.nn.utils.rnn.pad_sequence(points_data_list)
            points_list = []
            for i in range(points_data_list.shape[1]):
                points_list.append(points_data_list[:, i, ...])
        metas_map = {}
        ...
        for i, each in enumerate(queue):
            ...
            if metas_map[i]["scene_token"] != prev_scene_token:
                ...
                prev_pos = copy.deepcopy(metas_map[i]['can_bus'][:2])
                prev_angle = copy.deepcopy(metas_map[i]['can_bus'][3])
                metas_map[i]['can_bus'][:2] = 0
                metas_map[i]['can_bus'][3] = 0
            else:
                metas_map[i]["prev_bev_exists"] = True
                tmp_pos = copy.deepcopy(metas_map[i]['can_bus'][:2])
                tmp_angle = copy.deepcopy(metas_map[i]['can_bus'][3])
                metas_map[i]['can_bus'][:2] -= prev_pos
                metas_map[i]['can_bus'][3] -= prev_angle
    ...
    def get_data_info(self, index):
        ...
        input_dict = dict(
            ...
            can_bus_filename=info["can_bus_path"],
            sweeps=[],
        ...
        roads = self.get_road_info(index)
        input_dict["road_info"] = roads
        can_bus = self.get_can_bus(index)
    ...
    def get_road_info(self, index):
        """Get road info according to the given index.

        Args:
            index (int): Index of the road data to get.

        Returns:
            dict: Road information consists of the following keys:
                - road_info (np.array): Result of road perception.
        """
        info = self.data_infos[index]
        road_results = []

        with open(info["road_path"],'r') as f:
            road_dict = json.load(f)

        for obj in road_dict["object"]:
            d = math.sqrt(obj["x"]**2+obj["y"]**2)
            if d <= 50:
                road_result = [obj["delay"], obj["x"], obj["y"], obj["yaw"], obj["velocity"], obj["acceleration"], obj["id"],]
                road_results.append(road_result)

        road_results = np.array(road_results, dtype=np.float32)
        return road_results
    
    def get_can_bus(self, index):
        """Get can bus according to the given index.
        Coordinate : UTM

        Args:
            index (int): Index of the can_bus data to get.

        Returns:
            dict: Can bus information consists of the following keys:
                - can_bus (np.array): Result of ego vehicle's can bus.
        """
        info = self.data_infos[index]

        with open(info["can_bus_path"],'r') as f:
            can_bus_dict = json.load(f)

        yaw_rad = can_bus_dict['yaw']
        yaw_deg = can_bus_dict['yaw'] / np.pi *180

        can_bus_result = [can_bus_dict['x'], can_bus_dict['y'], yaw_rad, yaw_deg, can_bus_dict['velocity'], can_bus_dict['yaw_rate'], can_bus_dict['acceleration']]

        can_bus_result = np.array(can_bus_result, dtype=np.float32)
        return can_bus_result
