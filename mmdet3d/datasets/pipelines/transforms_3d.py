...
class ObjectRangeFilter(object):
    ...
    def __call__(self, input_dict):
        ...
        if "road_info" in input_dict:
            road_info = input_dict["road_info"]
            if len(road_info) != 0:
                mask = (
                                (road_info[:, 0] > self.bev_range[0])
                                & (road_info[:, 1] > self.bev_range[1])
                                & (road_info[:, 0] < self.bev_range[2])
                                & (road_info[:, 1] < self.bev_range[3])
                )
                road_info = road_info[mask]
                input_dict["road_info"] = road_info
        return input_dict
