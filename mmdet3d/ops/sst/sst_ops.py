...
class SRATensor(object):
    ...
    def dense(self, channels_first=True):
        output_shape = [self.batch_size] + list(self.spatial_shape) + [self._features.shape[1]]
    ...
    def setup(self, batching_info, key, window_shape, temperature):
        ...
        mask_s1 = self.get_key_padding_mask(
            transform_info_s1, drop_lvl_s1, batching_info, self._features.device
        )
        mask_s2 = self.get_key_padding_mask(
            transform_info_s2, drop_lvl_s2, batching_info, self._features.device
        )
        ...
        pos_s1 = self.get_pos_embed(
            ...
            feat_dim,
            temperature,
            self._features.dtype,
        )
        pos_s2 = self.get_pos_embed(
            transform_info_s2,
            coors_in_win_s2,
            drop_lvl_s2,
            batching_info,
    ...
    def window_tensor(self, do_shift):
        ...
        assert self.ready
        assert (
            not self._transformed_to_window
        ), "window_tensor should not be called twice without update"
        assert self.dropped
        ...
        # def flat2window(feat, voxel_drop_lvl, flat2win_inds_dict, drop_info):
        window_tensor_dict = flat2window(
            self._features, drop_level, transform_info, self.batching_info
        )
    ...
    def window_partition(self, do_shift):
        ...
        batch_win_inds = (
            self.indices[:, 0] * max_num_win_per_sample + win_coors_x * max_num_win_y + win_coors_y
        )
    ...
    def get_key_padding_mask(self, transform_info, voxel_drop_level, batching_info, device):
        ...
        window_key_padding_dict = flat2window(
            key_padding, voxel_drop_level, transform_info, batching_info
        )
