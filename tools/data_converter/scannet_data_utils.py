...
class ScanNetData(object):
    ...
    def get_box_label(self, idx):
        box_file = osp.join(self.root_dir, "scannet_train_instance_data", f"{idx}_bbox.npy")
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            pts_filename = osp.join(
                self.root_dir, "scannet_train_instance_data", f"{sample_idx}_vert.npy"
            )
            pts_instance_mask_path = osp.join(
                self.root_dir, "scannet_train_instance_data", f"{sample_idx}_ins_label.npy"
            )
            pts_semantic_mask_path = osp.join(
                self.root_dir, "scannet_train_instance_data", f"{sample_idx}_sem_label.npy"
            )
            ...
            if has_label:
                annotations = {}
                boxes_with_classes = self.get_box_label(sample_idx)  # k, 6 + class
