...
class TestForeignKeys(unittest.TestCase):
    ...
    def test_foreign_keys(self) -> None:
        ...
        for table_name in self.nuim.table_names:
            ...
            # Check 1-to-1 link.
            one_to_one_names = [
                k for k in keys if k.endswith("_token") and not k.startswith("key_")
            ]
                    ...
                    if foreign_table_name == "category":
                        remove = set(
                            [
                                cat["token"]
                                for cat in self.nuim.category
                                if cat["name"] not in ["vehicle.ego", "flat.driveable_surface"]
                            ]
                        )
                        foreign_index = foreign_index.difference(remove)
                    elif foreign_table_name == "sample_data":
            ...
            for foreign_key_name in prev_next_names:
                ...
                foreign_tokens = set(
                    [row[foreign_key_name] for row in table if len(row[foreign_key_name]) > 0]
                )
    ...
    def test_prev_next(self) -> None:
        ...
        for sample in self.nuim.sample:
            ...
            self.assertTrue(
                set(sd_tokens_pointers) == set(sd_tokens_all),
            ...
            self.assertTrue(
                sorted(timestamps) == timestamps, "Error: Timestamps not properly sorted!"
            )
