...
class NuImages:
    ...
    def __getattr__(self, attr_name: str) -> Any:
        ...
        if attr_name in self.table_names:
            ...
            raise AttributeError(
                "Error: %r object has no attribute %r" % (self.__class__.__name__, attr_name)
            )
    ...
    def __load_table__(self, table_name) -> List[dict]:
        ...
        if self.verbose:
            print(
                "Loaded {} {}(s) in {:.3f}s,".format(len(table), table_name, end_time - start_time)
            )
    ...
    def shortcut(self, src_table: str, tgt_table: str, src_token: str) -> Dict[str, Any]:
        ...
        if src_table == "sample_data" and tgt_table == "sensor":
            ...
            raise Exception(
                "Error: Shortcut from %s to %s not implemented!" % (src_table, tgt_table)
            )
    ...
    def list_attributes(self, sort_by: str = "freq") -> None:
        ...
        if sort_by == "name":
            sort_order = [
                i for (i, _) in sorted(enumerate(self.attribute), key=lambda x: x[1]["name"])
            ]
        elif sort_by == "freq":
    ...
    def list_categories(
        ...
        if sort_by == "name":
            sort_order = [
                i for (i, _) in sorted(enumerate(self.category), key=lambda x: x[1]["name"])
            ]
    ...
    def list_anns(self, sample_token: str, verbose: bool = True) -> Tuple[List[str], List[str]]:
        ...
        if verbose:
            ...
            for object_ann in object_anns:
                ...
                attribute_names = [
                    self.get("attribute", at)["name"] for at in object_ann["attribute_tokens"]
                ]
    ...
    def list_sample_content(self, sample_token: str) -> None:
        ...
        timestamps = np.array(
            [self.get("sample_data", sd_token)["timestamp"] for sd_token in sample_data_tokens]
        )
    ...
    def render_image(
        ...
        ), "Error: {} is not a valid option for annotation_type. " "Only {} are allowed.".format(
            annotation_type, annotations_types
        )
