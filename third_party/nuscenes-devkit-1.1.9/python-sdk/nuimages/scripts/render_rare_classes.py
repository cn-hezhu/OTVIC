...
def render_rare_classes(
    ...
    # Checks.
    assert (
        "filter_categories" not in render_args.keys()
    ...
    filter_categories_freq = sorted(
        [k for (k, v) in object_freqs.items() if v / total_freqs <= max_frequency]
    )
    assert (
        len(filter_categories_freq) > 0
    ), "Error: No classes found with the specified max_frequency!"
