...
def render_images(
    ...
    assert not (
        out_type == "video" and mode == "trajectory"
    ...
    if filter_categories is not None:
        ...
        for category_name in filter_categories:
            assert category_name in category_names, (
                "Error: Invalid object_ann category %s!" % category_name
            )
    ...
    for sample_token in tqdm.tqdm(sample_tokens):
        ...
        if len(sd_tokens) < 13 and out_type == "video":
            print(
                "Warning: Skipping video for sample token %s, as not all 13 frames exist!"
                % sample_token
            )
            continue
        ...
        for mode in modes:
            out_path_prefix = os.path.join(
                out_dir, "%s_%s_%s_%s" % (log_name, sample_token, sample_cam_name, mode)
            )
            if out_type == "image":
...
def write_video(
    nuim: NuImages, sd_tokens: List[str], mode: str, out_path_prefix: str, cleanup: bool = True
) -> None:
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Render a random selection of images and save them to disk."
    )
    ...
    nuim_ = NuImages(
        version=args.version, dataroot=args.dataroot, verbose=bool(args.verbose), lazy=False
    )
