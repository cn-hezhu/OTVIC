...
def convert_annotation_list_to_dict(
    annotation_list: List[dict], categories: List[str] = None, visibilities: List[str] = None
) -> defaultdict:
    """
    ...
    for instance in annotation_list:
        ...
        if (
            instance_token in bbox_2d_annotations
            and sample_token in bbox_2d_annotations[instance_token]
            and camera_name in bbox_2d_annotations[instance_token][sample_token]
        ):
            num_dups += 1
            print(
                "Duplicate instance {}, sample {}, and camera {}".format(
...
def get_most_visible_camera_annotation(camera_data_dict: dict) -> dict:
    ...
    for camera_token in camera_data_dict:
        ...
        if visibility > best_visibility or (
            visibility == best_visibility and bbox_area > largest_area
        ):
...
def get_cropped_image_for_annotation(
    sample_data_annotation: dict, dataroot: str, output_size: Tuple[int, int]
) -> np.ndarray:
    """
    ...
    """
    data_path = os.path.join(dataroot, sample_data_annotation["filename"])
...
def sort_sample_annotations_chronologically(instance_dict: dict) -> List[str]:
    ...
    while True:
        try:
            next_sample_token = instance_dict[sequential_sample_tokens[-1]]["best_annotation"][
                "next"
            ]
        except:
...
def remove_bad_samples(
    instance_annotation: dict,
    minimum_bb_area: float,
    minimum_visibility: str,
    image_area: int = 1600 * 900,
) -> List:
    """
...
def main(
    ...
    if not os.path.exists(image_annotations_file):
        raise Exception(
            "Error: Missing image_annotations.json. "
    ...
    for instance_token in bbox_2d_annotations:
        for sample_annotation_token in bbox_2d_annotations[instance_token]:
            bbox_2d_annotations[instance_token][sample_annotation_token][
                "best_annotation"
            ] = get_most_visible_camera_annotation(
                bbox_2d_annotations[instance_token][sample_annotation_token]
            )
    # ==============================================================================================
    ...
    for instance_token in bbox_2d_annotations:
        bbox_2d_annotations[instance_token][
            "sample_annotation_sequence"
        ] = sort_sample_annotations_chronologically(bbox_2d_annotations[instance_token])
    # ==============================================================================================
    ...
    for instance_token in tqdm(bbox_2d_annotations):
        ...
        video_path = os.path.join(output, "{}.{}".format(instance_token, file_ext))
        out = cv2.VideoWriter(video_path, cv2.VideoWriter_fourcc(*codec), fps, output_size)

        for sample_annotation_token in sample_annotation_tokens:
            best_annotation = bbox_2d_annotations[instance_token][sample_annotation_token][
                "best_annotation"
            ]
            cropped_img = get_cropped_image_for_annotation(best_annotation, dataroot, output_size)
    ...
    print(
        "Created {} videos ({} did not meet requirements).".format(
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-v", "--version", type=str, default="v1.0-trainval", help="The nuScenes data version"
    )
    ap.add_argument("-o", "--output", type=str, default="videos", help="The output video directory")
    ap.add_argument(
        "-x",
        "--codec",
        type=str,
    ...
    ap.add_argument(
        "-s", "--size", type=int, default=(112, 112), nargs=2, help="Size of the output video"
    )
    ...
    ap.add_argument(
        ...
        "--categories",
        nargs="+",
        help="The categories to extract videos for",
        required=False,
