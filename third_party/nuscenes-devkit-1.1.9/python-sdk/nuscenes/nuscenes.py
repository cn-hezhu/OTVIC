...
class NuScenes:
    ...
    def __init__(
        ...
        for i, lidar_task in enumerate(lidar_tasks):
            ...
            num_label_files = len(
                [name for name in label_files if (name.endswith(".bin") or name.endswith(".npz"))]
            )
        ...
        for map_record in self.map:
            map_record["mask"] = MapMask(
                osp.join(self.dataroot, map_record["filename"]), resolution=map_resolution
            )
...
class NuScenesExplorer:
    ...
    def list_panoptic_instances(self, sort_by: str = "count", get_hist: bool = False) -> None:
        ...
        for frame_id, record_panoptic in enumerate(nusc_panoptic):
            ...
            sample_token = self.nusc.get("sample_data", record_panoptic["sample_data_token"])[
                "sample_token"
            ]
        ...
        if sort_by == "count":
            instance_counts = sorted(
                instance_counts.items(), key=lambda item: item[1]["num_instances"], reverse=True
            )
        elif sort_by == "name":
        ...
        print(
            ...
            "\n=====".format(
                len(nusc_panoptic), time.time() - start_time, num_instances, num_sample_annos
            )
        )
    ...
    def list_scenes(self) -> None:
        ...
        for start_time, record in sorted(recs):
            start_time = (
                self.nusc.get("sample", record["first_sample_token"])["timestamp"] / 1000000
            )
            length_time = (
                self.nusc.get("sample", record["last_sample_token"])["timestamp"] / 1000000
                - start_time
            )
    ...
    def render_ego_centric_map(
        ...
        cropped = crop_image(
            mask_raster, pixel_coords[0], pixel_coords[1], int(scaled_limit_px * math.sqrt(2))
        )
    ...
    def render_scene_channel_lidarseg(
        ...
        assert channel in valid_channels, "Error: Input camera channel {} not valid.".format(
            channel
        )
        assert imsize[0] / imsize[1] == 16 / 9, "Error: Aspect ratio should be 16/9."
        ...
        while keep_looping:
            ...
            cam = self.nusc.get("sample_data", camera_token)
            filename = (
                scene_record["name"] + "_" + channel + "_" + os.path.basename(cam["filename"])
            )
