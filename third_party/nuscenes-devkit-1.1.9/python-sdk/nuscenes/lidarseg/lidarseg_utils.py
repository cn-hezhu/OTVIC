...
def plt_to_cv2(
    points: np.array, coloring: np.array, im, imsize: Tuple[int, int] = (640, 360), dpi: int = 100
):
    """
...
def colormap_to_colors(colormap: Dict[str, Iterable[int]], name2idx: Dict[str, int]) -> np.ndarray:
    ...
    for i, (k, v) in enumerate(colormap.items()):
        ...
        assert (
            i == name2idx[k]
        ), "Error: {} is of index {}, " "but it is of index {} in the colormap.".format(
            k, name2idx[k], i
        )
        colors.append(v)
...
def paint_points_label(
    ...
    if filter_lidarseg_labels is not None:
        ...
        ), "All class indices in filter_lidarseg_labels should " "be between 0 and {}".format(
            len(name2idx) - 1
        )
