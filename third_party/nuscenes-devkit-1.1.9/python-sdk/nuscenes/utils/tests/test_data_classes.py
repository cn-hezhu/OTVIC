...
class TestDataClasses(unittest.TestCase):
    def test_load_pointclouds(self):
        ...
        pc3, _ = LidarPointCloud.from_file_multisweep(
            nusc, sample_rec, "LIDAR_TOP", "LIDAR_TOP", nsweeps=2
        )
        pc4, _ = RadarPointCloud.from_file_multisweep(
            nusc, sample_rec, "RADAR_FRONT", "RADAR_FRONT", nsweeps=2
        )

        # Check for valid dimensions.
        assert (
            pc1.points.shape[0] == pc3.points.shape[0] == 4
        ), "Error: Invalid dimension for lidar pointcloud!"
        assert (
            pc2.points.shape[0] == pc4.points.shape[0] == 18
        ), "Error: Invalid dimension for radar pointcloud!"
