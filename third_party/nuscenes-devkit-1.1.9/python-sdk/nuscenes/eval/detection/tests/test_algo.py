...
class TestAlgo(unittest.TestCase):
    ...
    def test_nd_score(self):
        ...
        for class_name in self.cfg.class_names:
            for dist_th in self.cfg.dist_ths:
                ap = calc_ap(
                    mdl[(class_name, dist_th)], self.cfg.min_recall, self.cfg.min_precision
                )
...
def get_metric_data(
    gts: Dict[str, List[Dict]], preds: Dict[str, List[Dict]], detection_name: str, dist_th: float
) -> DetectionMetricData:
    """
...
class TestAPSimple(unittest.TestCase):
    ...
    def test_one_sample(self):
        ...
        # Perfect detection.
        self.check_ap(
            {"sample1": [self.car1]}, {"sample1": [self.car1]}, target_ap=1.0, detection_name="car"
        )
...
class TestTPSimple(unittest.TestCase):
    ...
    def check_tp(
        ...
        self.assertGreaterEqual(
            0.01, abs(tp_error - target_error), msg="Incorrect {} value".format(metric_name)
        )
    ...
    def test_no_positives(self):
        ...
        for metric_name in TP_METRICS:
            self.check_tp(
                {"sample1": [car1]}, {"sample1": [car2]}, target_error=1.0, metric_name=metric_name
            )

        # Within distance threshold away but different classes.
        for metric_name in TP_METRICS:
            self.check_tp(
                {"sample1": [car1]}, {"sample1": [bike1]}, target_error=1.0, metric_name=metric_name
            )
    ...
    def test_perfect(self):
        ...
        for metric_name in TP_METRICS:
            ...
            self.check_tp(
                {"sample1": [car1]}, {"sample1": [car1]}, target_error=0.0, metric_name=metric_name
            )

            # Detected with low score.
            self.check_tp(
                {"sample1": [car1]}, {"sample1": [car2]}, target_error=0.0, metric_name=metric_name
            )
    ...
    def test_one_img(self):
        ...
        self.check_tp(
            {"sample1": [gt2]}, {"sample1": [pred2]}, target_error=1, metric_name="trans_err"
        )
