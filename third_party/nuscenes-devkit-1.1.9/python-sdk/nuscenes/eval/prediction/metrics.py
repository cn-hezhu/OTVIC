...
@returns_2d_array
def mean_distances(stacked_trajs: np.ndarray, stacked_ground_truth: np.ndarray) -> np.ndarray:
    """
...
def min_ade_k(
    stacked_trajs: np.ndarray, stacked_ground_truth: np.ndarray, mode_probabilities: np.ndarray
) -> np.ndarray:
...
class MissRateTopK(Metric):
    def __init__(
        self, k_to_report: List[int], aggregators: List[Aggregator], tolerance: float = 2.0
    ):
        """
    ...
    def __call__(self, ground_truth: np.ndarray, prediction: Prediction) -> np.ndarray:
        ...
        results = miss_rate_top_k(
            prediction.prediction, ground_truth, prediction.probabilities, self.tolerance
        )
        return desired_number_of_modes(results, self.k_to_report)
...
class OffRoadRate(Metric):
    ...
    def load_drivable_area_masks(helper: PredictHelper) -> Dict[str, np.ndarray]:
        ...
        for map_name, map_api in maps.items():
            ...
            masks[map_name] = map_api.get_map_mask(
                patch_box=None, patch_angle=0, layer_names=["drivable_area"], canvas_size=None
            )[0]
    ...
    def serialize(self) -> Dict[str, Any]:
        return {"name": self.name, "aggregators": [agg.serialize() for agg in self.aggregators]}
...
def deserialize_metric(config: Dict[str, Any], helper: PredictHelper) -> Metric:
    ...
    if config["name"] == "MinADEK":
        return MinADEK(
            config["k_to_report"], [deserialize_aggregator(agg) for agg in config["aggregators"]]
        )
