...
OVERLAP_THRESHOLD = (
    0.5  # Amount by which an instance can overlap with other instances, before it is discarded.
)
...
def generate_panoptic_labels(
    ...
    for sample_token in tqdm(sample_tokens, disable=not verbose):
        ...
        for instance_id, (pred_box, cl, tracking_id) in enumerate(
            zip(sorted_pred_boxes, pred_cls, tracking_ids)
        ):
            cl_id = coarse2idx[cl]
        ...
        stuff_msk = np.logical_and(
            panop_labels == 0, lidar_seg.labels >= STUFF_START_COARSE_CLASS_ID
        )
        panop_labels[stuff_msk] = lidar_seg.labels[stuff_msk] * 1000
        panoptic_file = sd_record["token"] + "_panoptic.npz"
        np.savez_compressed(
            os.path.join(panoptic_dir, panoptic_file), data=panop_labels.astype(np.uint16)
        )


def sort_confidence(
    boxes: List[Union[DetectionBox, TrackingBox]]
    ...
    """
    scores = [
        box.tracking_score if isinstance(box, TrackingBox) else box.detection_score for box in boxes
    ]
    ...
    sorted_cls = [
        box.tracking_name if isinstance(box, TrackingBox) else box.detection_name
        for box in sorted_bboxes
    ]

    tracking_ids = [
        box.tracking_id if isinstance(box, TrackingBox) else None for box in sorted_bboxes
    ]
