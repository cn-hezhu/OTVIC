...
class PanopticTrackingEval(PanopticEval):
    ...
    @staticmethod
    def update_dict_stat(
        stat_dict: Dict[int, int], unique_ids: np.ndarray, unique_cnts: np.ndarray
    ) -> None:
        """
    ...
    def get_panoptic_track_stats(
        ...
        np.ndarray, np.ndarray, np.ndarray, np.ndarray, Dict[int, int], Dict[int, int], np.ndarray
    ]:
        """
        ...
        if scene is not None and cl < self.min_stuff_cls_id:
            ...
            valid_combos_min_point = np.zeros_like(
                y_inst_in_cl
            )  # instances which have more than self.min points
            ...
            for idx, value in enumerate(tp_indexes_agnostic):
                if value:
                    ...
                    if g_label not in self.instance_gts[scene][cl]:
                        self.instance_gts[scene][cl][g_label] = [
                            p_label,
                        ]
                    else:
    ...
    def add_batch_panoptic(
        ...
        for cl in self.include:
            ...
            if x_sem_row[0] is not None:  # First frame.
                ...
                _, _, gt_labels_prev, inst_prev, _, _, ious = self.get_panoptic_track_stats(
                    x_inst_in_cl, y_inst_in_cl
                )
            ...
            self.pan_fn[cl] += np.sum(
                np.logical_and(counts_gt >= self.min_points, np.logical_not(matched_gt))
            )
            # Count the FP.
            self.pan_fp[cl] += np.sum(
                np.logical_and(counts_pred >= self.min_points, np.logical_not(matched_pred))
            )
            ...
            if x_sem_row[0] is not None and cl < self.min_stuff_cls_id:  # Skip first frame.
                ...
                _, prev_ind, cur_ind = np.intersect1d(
                    gt_labels_prev, gt_labels, return_indices=True
                )
    ...
    def get_ptq(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        ptq_all = ((iou - ids) / tp_eps) * (
            tp / tp_half_fp_half_fn_eps
        )  # Calculate PTQ of all classes.
        soft_ptq_all = ((iou - soft_ids) / tp_eps) * (
            tp / tp_half_fp_half_fn_eps
        )  # Calculate soft-PTQ of all classes.
    ...
    def get_motsa(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        tp_eps, fn = np.maximum(tp, self.eps), self.pan_fn[1 : self.min_stuff_cls_id].astype(
            np.double
        )
        ...
        for i in range(valid_classes.shape[0]):
            if (
                i + 1 not in self.include
            ):  # i + 1 as valid_clssses covers class IDs of 1:self.min_stuff_cls_id.
                valid_classes[i] = False
    ...
    def get_lstq(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        s_assoc = np.sum(self.pan_aq) / np.sum(
            num_tubes[1 : self.min_stuff_cls_id]
        )  # num_things 1:11.
        ...
        for seq in self.sequences:
            ...
            for cl in self.include:
                ...
                for gt_id, pr_ids in cls_gts.items():
                    ...
                    unique_pr_id, counts_pr_id = (
                        unique_pr_id[unique_pr_id != 1],
                        counts_pr_id[unique_pr_id != 1],
                    )
                    fp_pr_id = []
                    ...
                    gt_id_aq = np.sum(
                        counts_pr_id**2 / np.double(track_length + fp_pr_id)
                    ) / np.double(track_length)
