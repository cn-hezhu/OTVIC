...
def prepare_files(method_names: List[str], root_dir: str) -> None:
    ...
    for method_name in method_names:
        ...
        ), "Error: Zip file for method {} does not exist at {}.".format(
            method_name, zip_path_to_predictions_by_method
        )
...
def get_prediction_json_path(prediction_dir: str) -> str:
    ...
    assert (
        len(files_in_dir) == 1
    ), "Error: The submission .zip file must contain exactly one .json file."
...
def panop_baselines_from_lidarseg_detect_track(
    out_dir: str,
    ...
    with joblib.Parallel(n_jobs=n_jobs) as parallel:
        parallel(
            [
                joblib.delayed(generate_and_evaluate_baseline)(
                    out_dir,
                    lidarseg_preds_dir,
    ...
    print(
        "Generated and evaluated {} panoptic {} baselines in {} seconds.".format(
...
def generate_and_evaluate_baseline(
    ...
    evaluator.evaluate()
    print(
        "Evaluation for panoptic {} using predictions merged from {} and {} saved at {}.".format(
            task, lidarseg_method_name, det_or_track_method_name, dir_to_save_evaluation_results_to
        )
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Create baselines for a panoptic task (tracking or segmentation)."
    )
    ...
    parser.add_argument(
        "--verbose", type=bool, default=False, help="Whether to print messages to stdout."
    )

    args = parser.parse_args()
    print(args)

    panop_baselines_from_lidarseg_detect_track(
