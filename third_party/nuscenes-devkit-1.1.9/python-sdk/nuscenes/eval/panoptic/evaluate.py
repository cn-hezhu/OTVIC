...
class NuScenesPanopticEval:
    ...
    def __init__(
        ...
        """
        assert (
            hasattr(nusc, "panoptic") and len(getattr(nusc, "panoptic")) > 0
        ...
        self.id2name = {
            idx: name for name, idx in self.mapper.coarse_name_2_coarse_idx_mapping.items()
        }
        ...
        if self.task == "tracking":
            ...
            self.evaluator["tracking"] = PanopticTrackingEval(
                n_classes=self.num_classes,
    ...
    def evaluate_segmentation(self) -> Dict[str, Any]:
        ...
        for sample_token in tqdm(self.sample_tokens, disable=not self.verbose):
            ...
            panoptic_label_filename = os.path.join(
                self.nusc.dataroot, self.nusc.get("panoptic", sd_token)["filename"]
            )
            ...
            panoptic_pred_filename = os.path.join(
                self.results_folder, "panoptic", self.eval_set, sd_token + "_panoptic.npz"
            )
        ...
        mean_pq, mean_sq, mean_rq, class_all_pq, class_all_sq, class_all_rq = self.evaluator[
            "segmentation"
        ].getPQ()
        mean_iou, class_all_iou = self.evaluator["segmentation"].getSemIoU()

        results = self.wrap_result_segmentation(
            mean_pq,
            mean_sq,
            mean_rq,
            mean_iou,
    ...
    def wrap_result_segmentation(
        ...
        """
        mean_pq, mean_sq, mean_rq, mean_iou = (
            mean_pq.item(),
            mean_sq.item(),
            mean_rq.item(),
            mean_iou.item(),
    ...
    def evaluate_tracking(self) -> Dict[str, Any]:
        ...
        for scene in tqdm(eval_scenes, disable=not self.verbose):
            ...
            while True:
                ...
                self.evaluator["tracking"].add_batch(
                    scene["name"], pred_sem, pred_inst, label_sem, label_inst
                )
...
def main():
    ...
    parser.add_argument(
        "--out_dir", type=str, default=None, help="Folder to write the panoptic labels to."
    )
    ...
    evaluator = NuScenesPanopticEval(
        nusc=nusc,
    ...
    evaluator.evaluate()
    print(
        f"Evaluation results saved at {args.out_dir}/{task}-result.json. \nFinished {task} evaluation."
    )
