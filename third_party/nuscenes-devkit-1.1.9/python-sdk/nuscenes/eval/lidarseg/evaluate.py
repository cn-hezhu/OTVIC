...
from nuscenes.eval.lidarseg.utils import (
    ConfusionMatrix,
    LidarsegClassMapper,
...
class LidarSegEval:
    ...
    def __init__(self, nusc: NuScenes, results_folder: str, eval_set: str, verbose: bool = False):
        ...
        assert len(nusc.lidarseg) > 0, "Error: No ground truth annotations found in {}.".format(
            nusc.version
        )
        ...
        ), "Error: The folder containing the .bin files ({}) does not exist.".format(
            self.results_bin_folder
        )
