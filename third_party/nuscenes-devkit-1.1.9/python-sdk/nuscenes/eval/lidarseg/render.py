...
class LidarSegEvalStratified(LidarSegEval):
    ...
    def __init__(
        ...
        self.strata_names = [
            "{}m to {}m".format(stratum[0], stratum[1])
            if stratum[1] is not None
            else str(stratum[0]) + "m+"
        ...
        self.global_cm = [
            ConfusionMatrix(self.num_classes, self.ignore_idx) for i in range(len(strata_list))
        ]
    ...
    def evaluate(self) -> None:
        ...
        for i, stratum in enumerate(self.strata_list):
            ...
            for sample_token in tqdm(self.sample_tokens, disable=not self.verbose):
                ...
                gt_path = os.path.join(
                    self.nusc.dataroot, self.nusc.get("lidarseg", sd_token)["filename"]
                )
                ...
                pred_path = os.path.join(
                    self.results_folder, "lidarseg", self.eval_set, sd_token + "_lidarseg.bin"
                )
                ...
                pred = self.filter_pointcloud_by_depth(
                    pred, min_depth=stratum[0], max_depth=stratum[1]
                )
        ...
        if self.output_dir:
            self.render_stratified_per_class_metrics(
                os.path.join(self.output_dir, "stratified_iou_per_class.png")
            )
            self.render_stratified_overall_metrics(
                os.path.join(self.output_dir, "stratified_iou_overall.png")
            )
    ...
    def render_stratified_overall_metrics(self, filename: str, dpi: int = 100) -> None:
        ...
        """
        stratified_iou = {
            stratum_name: self.stratified_per_class_metrics[i]["miou"]
            for i, stratum_name in enumerate(self.strata_names)
        }

        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.bar(list(stratified_iou.keys()), list(stratified_iou.values()), color="grey")
        ...
        for j, rect in enumerate(ax.patches):
            ax.text(
                ...
                rect.get_y() + rect.get_height() + 0.01,
    ...
    def render_stratified_per_class_metrics(self, filename: str, dpi: int = 250) -> None:
        ...
        for n, (cls, cls_strata) in enumerate(stratified_classes.items()):
            ...
            ax.bar(
                self.strata_names,
                cls_strata,
                color=np.array(self.mapper.coarse_colormap[cls]) / 255,
            )
            ax.spines["right"].set_visible(False)
            ax.spines["top"].set_visible(False)
            ax.set_xticklabels(self.strata_names, rotation=45, horizontalalignment="right")
    ...
    def get_stratified_per_class_metrics(self) -> List[Dict]:
        ...
        for stratum_cm in self.global_cm:
            ...
            results = {
                "iou_per_class": {
                    self.id2name[i]: class_iou for i, class_iou in enumerate(iou_per_class)
                },
                "miou": miou,
    ...
    @staticmethod
    def filter_pointcloud_by_depth(
        pc: LidarSegPointCloud, min_depth: float = 0, max_depth: float = None
    ) -> LidarSegPointCloud:
        """
...
def visualize_semantic_differences_bev(
    nusc: NuScenes,
    sample_token: str,
...
if __name__ == "__main__":
    ...
    evaluator = LidarSegEvalStratified(
        nusc_, result_path_, out_path_, eval_set=eval_set_, verbose=verbose_
    )
    evaluator.evaluate()
