...
class ConfusionMatrix:
    ...
    def _get_confusion_matrix(self, gt_array: np.ndarray, pred_array: np.ndarray) -> np.ndarray:
        ...
        ), "Error: Array for ground truth must be between 0 and {} (inclusive).".format(
            self.num_classes - 1
        )
        assert all(
            (pred_array > 0) & (pred_array < self.num_classes)
        ), "Error: Array for predictions must be between 1 and {} (inclusive).".format(
            self.num_classes - 1
        )
...
class LidarsegClassMapper:
    ...
    def get_coarse2idx(self) -> Dict[str, int]:
        ...
        """
        return {
            self.ignore_class["name"]: self.ignore_class["index"],
            "barrier": 1,
            "bicycle": 2,
            "bus": 3,
            "car": 4,
    ...
    def get_coarse2color(self) -> Dict[str, Tuple[int]]:
        ...
        """
        return {
            self.ignore_class["name"]: (0, 0, 0),  # Black.
    ...
    def check_mapping(self) -> None:
        ...
        assert coarse_set == set(
            self.coarse_name_2_coarse_idx_mapping.keys()
    ...
    def convert_label(self, points_label: np.ndarray) -> np.ndarray:
        ...
        assert self.compare_stats(counter_before, counter_after), (
            "Error: Statistics of labels have changed " "after conversion. Pls check."
        )
    ...
    def get_stats(self, points_label: np.array) -> List[int]:
        ...
        lidarseg_counts = [0] * (
            max(
                max(self.fine_idx_2_coarse_idx_mapping.keys()),
                max(self.fine_idx_2_coarse_idx_mapping.values()),
            )
            + 1
        )
