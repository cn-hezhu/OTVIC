...
def validate_submission(
    nusc: NuScenes, results_folder: str, eval_set: str, verbose: bool = False, zip_out: str = None
) -> None:
    """
    ...
    assert os.path.exists(
        results_meta_folder
    ), "Error: The folder containing the submission.json ({}) does not exist.".format(
        results_meta_folder
    )
    ...
    with open(submisson_json_path) as f:
        ...
        assert valid_meta == set(submission_meta["meta"].keys()), "{} must contain {}.".format(
            submisson_json_path, valid_meta
        )
    ...
    for sample_token in tqdm(sample_tokens, disable=not verbose):
        ...
        if (
            len(nusc.lidarseg) > 0
        ):  # If ground truth exists, compare the no. of predictions with that of ground truth.
            lidarseg_label_filename = os.path.join(
                nusc.dataroot, nusc.get("lidarseg", sd_token)["filename"]
            )
            assert os.path.exists(
                lidarseg_label_filename
            ), "Error: The ground truth .bin file {} does not exist.".format(
                lidarseg_label_filename
            )
        ...
        assert num_points == len(lidarseg_pred), (
            ...
            "but there are only {} points in the point cloud.".format(
                len(lidarseg_pred), sd_token, num_points
            )
        )

        assert all(
            (lidarseg_pred > 0) & (lidarseg_pred < num_classes)
        ), "Error: Array for predictions in {} must be between 1 and {} (inclusive).".format(
            lidarseg_pred_filename, num_classes - 1
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--zip_out", type=str, default=None, help="Path to zip the results folder to."
    )
