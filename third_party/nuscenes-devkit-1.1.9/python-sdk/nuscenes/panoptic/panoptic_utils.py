...
def generate_panoptic_colors(
    colormap: Dict[str, Iterable[int]], name2idx: Dict[str, int], max_instances: int = 32000
) -> np.ndarray:
    """
...
def get_frame_panoptic_instances(panoptic_label: np.ndarray, frame_id: int = None) -> np.ndarray:
    ...
    cat_inst_count = np.concatenate(
        [inst_count[:, 0:1] // 1000, inst_count[:, 0:1] % 1000, inst_count[:, 1:]], axis=1
    ).astype(np.int32)
...
def get_panoptic_instances_stats(
    scene_inst_stats: Dict[str, np.ndarray], cat_idx2name: Dict[int, str], get_hist: bool = False
) -> Dict[str, Any]:
    """
    ...
    mean_inst_num_per_frame, std_inst_num_per_frame = np.mean(inst_num_each_frame), np.std(
        inst_num_each_frame
    )
    ...
    for cat_id in unique_cat_ids:
        ...
        per_cat_inst_frame_count = np.array(np.unique(per_cat_unique_inst_ids, return_counts=True))[
            1
        ]
