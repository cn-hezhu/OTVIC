...
class NuScenesCanBus:
    ...
    def __init__(self, dataroot: str = "/data/sets/nuscenes", max_misalignment: float = 5.0):
        """
    ...
    def print_all_message_stats(self, scene_name: str, print_full: bool = False) -> None:
        """
        ...
        if print_full:
            ...
            partial_messages = {
                message: list(stats.keys()) for message, stats in all_messages.items()
            }
        ...
        if scene_id in self.can_blacklist:
            ...
            if message_name == "route":
                if scene_id in self.route_blacklist:
                    warnings.warn(
                        "Warning: %s is not well aligned with the baseline route!" % scene_name
                    )
