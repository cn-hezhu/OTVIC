...
class MockNuScenes:
    ...
    def __init__(self, sample_annotations: List[Dict[str, Any]], samples: List[Dict[str, Any]]):
...
class TestConvertCoords(unittest.TestCase):
    ...
    def test_heading_0(self):
        ...
        np.testing.assert_allclose(
            convert_local_coords_to_global(answer, origin, rotation), self.along_pos_x, atol=1e-4
        )
        ...
        np.testing.assert_allclose(
            convert_local_coords_to_global(answer, origin, rotation), self.along_pos_y, atol=1e-4
        )
        ...
        np.testing.assert_allclose(
            convert_local_coords_to_global(answer, origin, rotation), self.along_neg_x, atol=1e-4
        )
        ...
        np.testing.assert_allclose(
            convert_local_coords_to_global(answer, origin, rotation), self.along_neg_y, atol=1e-4
        )
...
class TestPredictHelper(unittest.TestCase):
    ...
    def test_get_future_for_agent_no_data_to_get(
        ...
        mock_samples = [{"token": "1", "timestamp": 0}, {"token": "2", "timestamp": 3.5e6}]
    ...
    def test_get_past_for_agent_in_frame(
        ...
        np.testing.assert_allclose(past, np.array([[1.0, -1.0], [2.0, -2.0], [3.0, -3.0]]))
    ...
    def test_get_past_for_agent_no_data_to_get(
        ...
        mock_samples = [{"token": "5", "timestamp": 0}, {"token": "4", "timestamp": -3.5e6}]
    ...
    def test_get_future_for_sample(self):
        ...
        answer = {"1": np.array([[1, 1], [2, 2], [3, 3]]), "2": np.array([[7, 7], [8, 8], [9, 9]])}
    ...
    def test_velocity_return_nan_big_diff(self):
        mock_samples = [{"token": "1", "timestamp": 0}, {"token": "2", "timestamp": 2.5e6}]
        nusc = MockNuScenes(self.mock_annotations, mock_samples)
        helper = PredictHelper(nusc)
