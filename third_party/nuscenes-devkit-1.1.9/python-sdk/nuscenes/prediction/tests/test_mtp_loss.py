...
class TestMTPLoss(unittest.TestCase):
    ...
    def test_angle_between_trajectories(self):
        ...
        # test angle is 0.
        self.assertEqual(loss._angle_between(make_trajectory([0, 0]), make_trajectory([0, 0])), 0.0)
        self.assertEqual(
            loss._angle_between(make_trajectory([15, 15]), make_trajectory([15, 15])), 0.0
        )

        # test angle is 15.
        self.assertAlmostEqual(
            loss._angle_between(make_trajectory([1, 1]), make_trajectory([math.sqrt(3) / 2, 0.5])),
            15.0,
            places=4,
        )
        ...
        # test angle is 30.
        self.assertAlmostEqual(
            loss._angle_between(make_trajectory([1, 0]), make_trajectory([math.sqrt(3) / 2, 0.5])),
            30.0,
            places=4,
        )
        ...
        # test angle is 45.
        self.assertAlmostEqual(
            loss._angle_between(make_trajectory([1, 1]), make_trajectory([0, 1])), 45.0, places=4
        )

        # test angle is 90.
        self.assertAlmostEqual(
            loss._angle_between(make_trajectory([1, 1]), make_trajectory([-1, 1])), 90.0, places=4
        )
        self.assertAlmostEqual(
            loss._angle_between(make_trajectory([1, 0]), make_trajectory([0, 1])), 90.0, places=4
        )

        # test angle is 180.
        self.assertAlmostEqual(
            loss._angle_between(make_trajectory([1, 0]), make_trajectory([-1, 0])), 180.0, places=4
        )
        self.assertAlmostEqual(
            loss._angle_between(make_trajectory([0, 1]), make_trajectory([0, -1])), 180.0, places=4
        )
        self.assertAlmostEqual(
            loss._angle_between(make_trajectory([3, 1]), make_trajectory([-3, -1])), 180.0, places=4
        )
    ...
    def test_loss_single_mode(self):
        ...
        self.assertAlmostEqual(float(loss(predictions, targets).detach().numpy()), 0, places=4)

        # Now the best mode differs by 1 from the ground truth.
        # Smooth l1 loss subtracts 0.5 from l1 norm if diff >= 1.
        predictions[:, :60] += 1
        self.assertAlmostEqual(float(loss(predictions, targets).detach().numpy()), 0.5, places=4)

        # In this case, one element has perfect regression, the others are off by 1.
        predictions[1, :60] -= 1
        self.assertAlmostEqual(
            float(loss(predictions, targets).detach().numpy()), (15 / 16) * 0.5, places=4
        )
    ...
    def test_loss_many_modes(self):
        ...
        self.assertAlmostEqual(
            float(loss(predictions, targets).detach().numpy()), -math.log(1 / 10), places=4
        )
        ...
        self.assertAlmostEqual(
            float(loss(predictions, targets).detach().numpy()), -math.log(1 / 10) + 0.5, places=4
        )
