...
def get_egoposes_on_drivable_ratio(
    nusc: NuScenes, nusc_map: NuScenesMap, scene_token: str
) -> float:
    """
...
def drop_disconnected_lanes(nusc_map: NuScenesMap) -> NuScenesMap:
    ...
    nusc_map.lane_connector = [
        lane for lane in nusc_map.lane_connector if lane["token"] not in disconnected
    ]
...
def get_disconnected_lanes(nusc_map: NuScenesMap) -> List[str]:
    ...
    for lane_token, connectivity in nusc_map.connectivity.items():
        ...
        for inout_lane_token in inout_lanes:
            if (
                inout_lane_token not in nusc_map._token2ind["lane"]
                and inout_lane_token not in nusc_map._token2ind["lane_connector"]
            ):
