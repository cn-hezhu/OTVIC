...
class NuScenesMap:
    ...
    def __init__(self, dataroot: str = "/data/sets/nuscenes", map_name: str = "singapore-onenorth"):
        """
        ...
        self.layer_names = (
            self.geometric_layers + self.lookup_polygon_layers + self.non_geometric_line_layers
        )
    ...
    def _get_stop_line_cue(self, stop_line_record: dict) -> List[dict]:
        ...
        if stop_line_record["stop_line_type"] in ["PED_CROSSING", "TURN_STOP"]:
            return [
                self.get("ped_crossing", token) for token in stop_line_record["ped_crossing_tokens"]
            ]
    ...
    def render_record(
        ...
        return self.explorer.render_record(
            layer_name, token, alpha, figsize=figsize, other_layers=other_layers, bitmap=bitmap
        )
    ...
    def get_records_in_radius(
        self, x: float, y: float, radius: float, layer_names: List[str], mode: str = "intersect"
    ) -> Dict[str, List[str]]:
        """
    ...
    def discretize_lanes(
        ...
        return {
            ID: discretize_lane(self.arcline_path_3.get(ID, []), resolution_meters) for ID in tokens
        }
...
class NuScenesMapExplorer:
                ...
                for polygon_token in polygon_tokens:
                    ...
                    ax.add_patch(
                        descartes.PolygonPatch(
                            polygon_proj, fc=self.color_map[layer_name], alpha=alpha, label=label
                        )
                    )
    ...
    def render_egoposes_on_fancy_map(
        ...
        for scene_token in tqdm(scene_tokens_location, disable=not verbose):
            ...
            if verbose and scene_id in scene_blacklist:
                print(
                    "Warning: %s is known to have a bad fit between ego pose and map." % scene_name
                )
    ...
    def _get_polygon_bounds(self, layer_name: str, token: str) -> Tuple[float, float, float, float]:
        ...
        if layer_name == "drivable_area":
            ...
            exterior_nodes = [
                self.map_api.get("node", token) for token in record["exterior_node_tokens"]
            ]
