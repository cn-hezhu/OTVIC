...
class AutoAssignHead(FCOSHead):
    ...
    def loss(
        ...
        for bbox_pred, gt_bboxe, inside_gt_bbox_mask in zip(
            ...
            expand_mlvl_points = (
                mlvl_points[:, None, :].expand(num_points, temp_num_gt, 2).reshape(-1, 2)
            )
            ...
            loss_bbox = self.loss_bbox(
                decoded_bbox_preds, decoded_target_preds, weight=None, reduction_override="none"
            )
    ...
    def get_targets(self, points, gt_bboxes_list):
        ...
        bbox_targets_list = [
            list(bbox_targets.split(num_points, 0)) for bbox_targets in bbox_targets_list
        ]
