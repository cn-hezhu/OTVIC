...
class Bottleneck(nn.Module):
    ...
    def __init__(
        self, in_channels, mid_channels, dilation, norm_cfg=dict(type="BN", requires_grad=True)
    ):
        super(Bottleneck, self).__init__()
        self.conv1 = ConvModule(in_channels, mid_channels, 1, norm_cfg=norm_cfg)
        self.conv2 = ConvModule(
            mid_channels, mid_channels, 3, padding=dilation, dilation=dilation, norm_cfg=norm_cfg
        )
