...
def generat_buckets(proposals, num_buckets, scale_factor=1.0):
    ...
    l_buckets = (
        px1[:, None]
        + (0.5 + torch.arange(0, side_num).to(proposals).float())[None, :] * bucket_w[:, None]
    )
    # right buckets
    r_buckets = (
        px2[:, None]
    ...
    t_buckets = (
        py1[:, None]
        + (0.5 + torch.arange(0, side_num).to(proposals).float())[None, :] * bucket_h[:, None]
    )
    # down buckets
    d_buckets = (
        py2[:, None]
...
def bbox2bucket(
    ...
    bucket_cls_weights = torch.cat(
        [bucket_cls_l_weights, bucket_cls_r_weights, bucket_cls_t_weights, bucket_cls_d_weights],
        dim=-1,
    )
