...
from mmdet.models.necks import FPN, ChannelMapper, CTResNetNeck, DilatedEncoder, SSDNeck, YOLOV3Neck
...
def test_ssd_neck():
    ...
    with pytest.raises(AssertionError):
        SSDNeck(
            in_channels=[8, 16], out_channels=[8, 16, 32], level_strides=[2], level_paddings=[2, 1]
        )
    ...
    with pytest.raises(AssertionError):
        SSDNeck(
            in_channels=[8, 16], out_channels=[4, 16, 64], level_strides=[2], level_paddings=[2]
        )

    ssd_neck = SSDNeck(
        in_channels=[4], out_channels=[4, 8, 16], level_strides=[2, 1], level_paddings=[1, 0]
    )
