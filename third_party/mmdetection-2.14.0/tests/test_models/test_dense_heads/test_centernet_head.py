...
def test_center_head_loss():
    ...
    empty_gt_losses = self.loss(
        center_out, wh_out, offset_out, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore
    )
...
def test_centernet_head_get_bboxes():
    ...
    detections = self.get_bboxes(
        [center_target], [wh_target], [offset_target], img_metas, rescale=True, with_nms=False
    )
